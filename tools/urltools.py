...
def read_url_get(url: str, headers: dict = None, data: dict = None, proxy: dict = None, _time=3):
    try:
        _url = url
        _headers = headers
        _data = data
        _proxy = proxy
        resp = requests.get(url, headers=_headers, data=_data)
    except HTTPError as e:
        return _read_url_retry(url, _time, f"HTTP 错误: {e.code}, URL: {url}")
    except URLError as e:
        return _read_url_retry(url, _time, f"URL 错误: {e.reason}, URL: {url}")
...
def _read_url_retry_get(url: str, headers: dict = None, data: dict = None, proxy: dict = None, _time=3, msg=''):
    if _time > 0:
        log.error(f'重试次数:{_time}, {msg}, url={url}')
        if _time > 2:
            time.sleep(5)
            ...
            time.sleep(20)
        return read_url_get(url=url, headers=headers, data=data, proxy=proxy, _time=3)
    else:
        return None
